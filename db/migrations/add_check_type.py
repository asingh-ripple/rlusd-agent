...
from db.database import Base, CheckType
from db.sqlite_config import get_connection_string

def run_migration():
    """Run the migration to add check_type column to checks table."""
    ...
    try:
        ...
        with conn.begin():
            ...
            # Create new table with check_type
            conn.execute(text("""
                CREATE TABLE checks_new (
                    ...
                    check_type VARCHAR NOT NULL DEFAULT 'CHECK_CREATE',
            ...
            conn.execute(text("""
                INSERT INTO checks_new (
                    check_id, transaction_hash, sender_id, receiver_id, 
                    amount, currency, expiration_date, insertion_date
                )
                SELECT 
                    check_id, transaction_hash, sender_id, receiver_id, 
                    amount, currency, expiration_date, insertion_date
